...
describe('ProductService', () => {
  ...
  describe('createProduct', () => {
    ...
    it('should throw 404 if store is not found', async () => {
      ...
      // Act & Assert
      await expect(productService.createProduct(productDto, userId)).rejects.toEqual(
        new CustomError(404, 'Store not found'),
      );
      expect(prismaService.store.findUnique).toHaveBeenCalledWith({
        where: { id: productDto.storeId },
      });
    ...
    it('should throw 403 if user does not own the store', async () => {
      ...
      // Act & Assert
      await expect(productService.createProduct(productDto, userId)).rejects.toEqual(
        new CustomError(403, 'You do not own this store'),
      );
      expect(prismaService.store.findUnique).toHaveBeenCalledWith({
        where: { id: productDto.storeId },
      });
    ...
    it('should throw 500 for unexpected errors', async () => {
      // Arrange
      mockPrismaService.store.findUnique.mockRejectedValue(new Error('Database error'));
  ...
  describe('findOneProduct', () => {
    ...
    const mockProduct = {
      ...
      reviews: [{ id: 'review-id-1', rating: 5, comment: 'Great product', createdAt: new Date() }],
      createdAt: new Date(),
      updatedAt: new Date(),
    };
  ...
  describe('updateProduct', () => {
    ...
    it('should update a product successfully', async () => {
      ...
      // Act
      const result = await productService.updateProduct(productId, productDto, userId);
    ...
    it('should throw 400 if ID is missing', async () => {
      // Act & Assert
      await expect(productService.updateProduct('', productDto, userId)).rejects.toEqual(
        new CustomError(400, 'Product ID is required'),
      );
      expect(prismaService.product.findUnique).not.toHaveBeenCalled();
    });
    ...
    it('should throw 404 if product is not found', async () => {
      ...
      // Act & Assert
      await expect(productService.updateProduct(productId, productDto, userId)).rejects.toEqual(
        new CustomError(404, 'Product not found'),
      );
      expect(prismaService.product.findUnique).toHaveBeenCalledWith({
        where: { id: productId },
    ...
    it('should throw 403 if user does not own the product’s store', async () => {
      ...
      // Act & Assert
      await expect(productService.updateProduct(productId, productDto, userId)).rejects.toEqual(
        new CustomError(403, 'You do not own this product'),
      );
      expect(prismaService.product.findUnique).toHaveBeenCalledWith({
        where: { id: productId },
      });
    ...
    it('should throw 404 if category is not found', async () => {
      ...
      // Act & Assert
      await expect(productService.updateProduct(productId, productDto, userId)).rejects.toEqual(
        new CustomError(404, 'Category not found'),
      );
      expect(prismaService.product.findUnique).toHaveBeenCalledWith({
        where: { id: productId },
      });
    ...
    it('should throw 404 if new store is not found', async () => {
      // Arrange
      mockPrismaService.product.findUnique.mockResolvedValue(mockProduct);
      mockPrismaService.store.findUnique.mockResolvedValueOnce(mockStore).mockResolvedValueOnce(null);
      mockPrismaService.category.findUnique.mockResolvedValue(mockCategory);
  ...
  describe('deleteProduct', () => {
    ...
    it('should throw 404 if product is not found', async () => {
      ...
      // Act & Assert
      await expect(productService.deleteProduct(productId, userId)).rejects.toEqual(
        new CustomError(404, 'Product not found'),
      );
      expect(prismaService.product.findUnique).toHaveBeenCalledWith({
        where: { id: productId },
    ...
    it('should throw 400 if product has associated order items', async () => {
      ...
      // Act & Assert
      await expect(productService.deleteProduct(productId, userId)).rejects.toEqual(
        new CustomError(400, 'Cannot delete product with associated order items'),
      );
      expect(prismaService.product.findUnique).toHaveBeenCalledWith({
        where: { id: productId },
        include: expect.any(Object),
